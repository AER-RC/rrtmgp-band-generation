      PROGRAM KDIS_2SORT_PLANCKFILTER
C***************************************************
...
C.. OF THE PLANCK INTEGRAL
           ...
           DO 4020 ILINE = J1, J2
                ...
                vv = v1 + (iline-1)*delv
                FILCNT = (VV - V1F) / DVF
                FILFRAC = FILCNT - FLOAT(INT(FILCNT))
                IFIL = 1 + INT(FILCNT - FILFRAC)
                FILVAL = FILTER(IFIL) + FILFRAC * 
     &               (FILTER(IFIL+1) - FILTER(IFIL))
                XCONT(ILINE-J1+1) = BBFN(VV,TAVE)*FILVAL
                vsum = vsum + xcont(iline-j1+1)
4020       CONTINUE

...
c     is tailored for the UPPER atmosphere rand handles
...
C***************************************************
      PARAMETER (NLINES=16000000,MLAYERS=47,MG=16)
...
c      if (iatmtype .eq. 1) then
c         level = igas_minor_l(igas_on,1)
c      else if (iatmtype .eq. 2) then
c         level = igas_minor_u(igas_on,1)
c      endif
...
c Must have total of all gases and broadener (N2)

           rhofac = (pave/tave)
           print*,'rhofac',level,pave,tave,rhofac,wbroad

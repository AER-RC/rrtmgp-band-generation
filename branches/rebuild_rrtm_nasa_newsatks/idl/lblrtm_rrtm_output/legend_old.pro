...
;       charthick =  character thickness used in legend labels
...
;-     Mark Shephard:
;          Nov. 2003- changed the default psym=8 to a circle  
;          Nov. 2003- commented out the color because it was interfering with
;                     subsequent plots after the legend was called 
;                     if psym(i) eq 8 then usersym,usersym,fill=fill ;,color=colors(i)
;         March 2010- added charthick and thick
pro legend_old,help=help,items,linestyle=linestyle,psym=psym,vectorfont=vectorfont $
...
IF N_ELEMENTS(chthick) LE 0 THEN chthick = 3.5
IF N_ELEMENTS(thick) LE 0 THEN thick = 3.5
...
if n_elements(usersym) eq 0 then BEGIN 
    ;;Making a circle symbol (psym=8)
    A = findgen(17) * (!PI*2/16.)
    usersym = TRANSPOSE([[cos(A)], [sin(A)]])
ENDIF 
...
for i = ii(0),ii(1),ii(2) do begin
  ...
  if psym(i) eq 8 then usersym,usersym,fill=fill, thick=chthick, color=colors(i)
...
;    if (num eq 1) and vertical then xp = x + xt/2	; IF 1, CENTERED.
    xyouts,xp,yp,vectorfont(i),width=width,color=colors(i) $
      ,charsize=charsize,align=xalign,/norm,charthick=chthick
    xt = xt > width
    xp = xp + width/2.
  endif else begin
    if symline and (linestyle(i) ge 0) then plots,xp,yp,color=colors(i) $
      ,/normal,linestyle=linestyle(i),psym=psym(i),symsize=charsize*.8, $
       thick=thick
  endelse
;  if i eq ii(1) then stop
  ...
  TEXT_ONLY:
  xyouts,x,y,delimiter,width=width,/norm,color=textcolors(i),charsize=charsize,align=xalign,charthick=chthick
  x = x + width*xsign
  if width ne 0 then x = x + 0.5*xspacing
  xyouts,x,y,items(i),width=width,/norm,color=textcolors(i),charsize=charsize,align=xalign,charthick=chthick
  x = x + width*xsign
...
if box then plots,[x,xend,xend,x,x],y + [0,0,ywidth,ywidth,0],/norm, thick=thick
return
end

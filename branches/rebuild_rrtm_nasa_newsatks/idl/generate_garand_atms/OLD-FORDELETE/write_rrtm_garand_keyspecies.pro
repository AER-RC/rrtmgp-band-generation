...
for iwvn = 0,n_elements(wvn1)-2 do begin
  scale_l = reform(key_l(iwvn,0:6))
  scale_u = reform(key_u(iwvn,0:6))

  IOUT = strcompress(iwvn+1,/remove_all)
  ...
  for j=0,ntrop-1 do begin
  	...
  	jcharp,jchart,jchar
  	printf,1,format='(7e10.4)',scale_l(0)*profile(0).wv(j),scale_l(1)*profile(0).co2(j),scale_l(2)*profile(0).o3(j), $
  		scale_l(3)*profile(0).n2o(j),scale_l(4)*profile(0).co(j),scale_l(5)*profile(0).ch4(j),scale_l(6)*profile(0).o2(j)
  endfor
  
  for j=ntrop,n_elements(zmdl)-1 do begin
  	printf,1,format='(3(f10.3),5x,a1,a1,1x,1x,1x,7a1)',zmdl(j),pm(j), tm(j),$
  	jcharp,jchart,jchar
    printf,1,format='(7e10.4)',scale_u(0)*profile(0).wv(j),scale_u(1)*profile(0).co2(j),scale_u(2)*profile(0).o3(j), $
  		scale_u(3)*profile(0).n2o(j),scale_u(4)*profile(0).co(j),scale_u(5)*profile(0).ch4(j),scale_u(6)*profile(0).o2(j)
  endfor

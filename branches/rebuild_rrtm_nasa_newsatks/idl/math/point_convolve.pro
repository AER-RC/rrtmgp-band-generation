...
if keyword_set(nofft) then begin
  ...
  ; maximum of 8 dimensions
    ...
    expanded_h[lowh[0]:highh[0], lowh[1]:highh[1], lowh[2]:highh[2], $
      lowh[3]:highh[3], lowh[4]:highh[4], lowh[5]:highh[5], $
      lowh[6]:highh[6], lowh[7]:highh[7]] = temporary(h)
    ...
    expanded_result = numexpandedcells * fft( fft(temporary(expanded_h))* $
      fft(temporary(expanded_kernel)), /inverse)
    ; fix the wraparound
    expanded_result = real_part(temporary(expanded_result))
    expanded_result=shift(temporary(expanded_result),numexpandedbins/2)
    result = temporary(expanded_result[lowh[0]:highh[0], $

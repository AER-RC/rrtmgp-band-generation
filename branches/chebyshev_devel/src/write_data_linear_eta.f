...
c Perform interpolation in eta
      OPEN(15,FILE='COMPARE_LINEAR_ETAS.txt',FORM='FORMATTED')
...
c            IF (SPECPARM .GE. ONEMINUS) THEN
c              KETA_2(JN,JT,JP,IG) = 
c     &          KA_2(JS,JT,JP,IG)
c              WRITE(15,7005) ETASTORED(JS),
c     &          SPECPARM,SPECPARM,-1.
c              WRITE(15,7010) KA_2(JS,JT,JP,IG),
c     &          KETA_2(JN,JT,JP,IG),
c     &          KREF_2(JN,JT,JP,IG),
c     &          -1.
c            ELSE
c              IF (SPECPARM .LT. 0.125) THEN
c                P = FS - 1
c                P4 = P**4
c                FK0 = P4
c                FK1 = 1 - P - 2.0*P4
c                FK2 = P + P4
c                KETA_2(JN,JT,JP,IG) = 
c     &            FK0*KA_2(JS,JT,JP,IG)+
c     &            FK1*KA_2(JS+1,JT,JP,IG)+
c     &            FK2*KA_2(JS+2,JT,JP,IG)
c              ELSE IF (SPECPARM .GT. 0.875) THEN
c                P = -FS
c                P4 = P**4
c                FK0 = P4
c                FK1 = 1 - P - 2.0*P4
c                FK2 = P + P4
c                KETA_2(JN,JT,JP,IG) = 
c     &            FK2*KA_2(JS-1,JT,JP,IG)+
c     &            FK1*KA_2(JS,JT,JP,IG)+
c     &            FK0*KA_2(JS+1,JT,JP,IG)
c              ELSE
                FS1 = 1.0 - FS
                KETA_2(JN,JT,JP,IG) = 
...
1625  CONTINUE     
      ...
      OPEN(UNIT=10,FORM='unformatted',FILE='COMPARE_LINEAR_ETAS.dat')
      WRITE(10) ETASTORED
      WRITE(10) KA_2
      WRITE(10) KETA_2
      WRITE(10) KREF_2

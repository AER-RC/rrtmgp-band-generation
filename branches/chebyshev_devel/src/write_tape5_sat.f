...
c Convert to number density
     ...
     &            PRESS(LEV),TEMP,
     &            SATRAT
                IF (SATRAT .GT. 1.05 .AND. IETA .LT. 9) THEN
                  IF (IETA .EQ. 8) ISAT(LEV,ITP) = 1
                  DENNUM = DENSAT*1.05
     ...
     &              *WS(2,LEV,ITP,IETA))
                ...
                IF (IETA .EQ. 9 .AND. ISAT(LEV,ITP) .EQ. 1) THEN
                  WS(1,LEV,ITP,9) = WS(1,LEV,ITP,8)
                  WRITE(25,*) 'CHANGING W @ 9:',WS(1,LEV,ITP,9)
                ENDIF

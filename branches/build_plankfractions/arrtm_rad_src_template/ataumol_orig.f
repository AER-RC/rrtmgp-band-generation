...
C     is interpolated (in temperature) separately.  
      ...
      WVNMEAN = (WAVENUM2(1)+WAVENUM1(1))/2.0 

      IF (WVNMEAN.GE.250. .AND. WVNMEAN.LE.500) THEN
         IBAND_LOGP=1
         PRINT*,'INTERPOLATION IN SQUARE ROOT OF LOG PRESSURE'
      ELSE
         IBAND_LOGP=0
         PRINT*,'INTERPOLATION IN LOG PRESSURE'
      ENDIF
...
C Lower Atmosphere - One Major Gas
      ...
      IF (IGAS1_L .EQ. 0 .AND. IGAS2_L .EQ. 0) THEN
         TAUG(1:LAYTROP,:) = 0.0
         FRACS(1:LAYTROP,:) = 0.0
...
c Add in minor gas contribution            
     ...
     &                 (pavel(lay)/pref(igas_minor_l(ii,1)))*
     &                 (tref(igas_minor_l(ii,1))/tavel(lay))
...
C Upper Atmosphere - One Major Gas
      IF (IGAS1_U .EQ.0 .AND. IGAS2_U .EQ. 0) THEN
         TAUG(LAYTROP+1,:) = 0.0
...
c upper atmosphere (level "13" of the tref atmosphere).
     ...
     &                 (pavel(lay)/pref(igas_minor_u(ii,1)+12))*
     &                 (tref(igas_minor_u(ii,1)+12)/tavel(lay))
